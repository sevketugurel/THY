...
# --- M5d LNS fold-redesign (plan: a-evet-ama-iki-tingly-canyon.md, adim 3):
# B fold -- yalnizca SERBEST instance/candidate'lar icin gercek Var/satir
# olusturur. Donuk (frozen) bir instance HIC Var olmuyor -- HiGHS'in
# presolve'unun bunu kesfedip elemesini beklemek yerine (bu, tam-model
# .fix() yolunun ~100s/iterasyonluk maliyetiydi), Python tarafinda bastan
# yok. Karisik bir aday (bir ucu serbest, digeri donuk) A'nin
# partial_rotation_rule deseniyle AYNI ilkeyle tek satir + sabit terim olur.
def add_flight_time_variables_folded(model, candidates, partition):
    arr_bounds, dep_bounds = {}, {}
    for c in candidates:
        if c.r1_id in partition.free_arr:
            if c.r1_id in arr_bounds:
                assert arr_bounds[c.r1_id] == (c.arr_lo, c.arr_hi), f"inconsistent bounds for {c.r1_id}"
            else:
                arr_bounds[c.r1_id] = (c.arr_lo, c.arr_hi)
        if c.r2_id in partition.free_dep:
            if c.r2_id in dep_bounds:
                assert dep_bounds[c.r2_id] == (c.dep_lo, c.dep_hi), f"inconsistent bounds for {c.r2_id}"
            else:
                dep_bounds[c.r2_id] = (c.dep_lo, c.dep_hi)
...
def add_b_constraints_folded(model, candidates, L: int, U: int, partition):
    """model.CANDIDATES only covers candidates with at least one free end
    (partition.is_free_candidate[i]) -- a fully-frozen candidate gets ZERO
    Vars/rows, its x/gap already known exactly via partition.x_const/
    gap_const. A 'mixed' candidate (one end free, one frozen) still gets a
    real row: gap_definition folds the frozen end in as a constant, exactly
    mirroring constraints_operations.py's add_a_constraints partial-pair
    pattern."""
    free_idx = [i for i in range(len(candidates)) if partition.is_free_candidate[i]]
    model.CANDIDATES = pyo.Set(initialize=free_idx, ordered=True)
    ...
    def _dep_term(m, c):
        return m.t_dep[c.r2_id] if c.r2_id in partition.free_dep else partition.reference_dep[c.r2_id]

    def _arr_term(m, c):
        return m.t_arr[c.r1_id] if c.r1_id in partition.free_arr else partition.reference_arr[c.r1_id]

    def gap_def_rule(m, i):
        c = candidates[i]
        return m.gap[i] == _dep_term(m, c) - _arr_term(m, c)
    model.gap_definition = pyo.Constraint(model.CANDIDATES, rule=gap_def_rule)
