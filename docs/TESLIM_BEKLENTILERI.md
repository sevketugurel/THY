...
| `python -m pytest` (data_raw/ **YOK** — jürinin ilk çalıştırması) | `344 passed, 4 skipped in ~18s` | **17.5–17.9s** |
| `python -m pytest` (data_raw/ **VAR** — geliştirme makinesi) | `348 passed in ~21-22s` | **21.1–21.7s** |
...
## 3 · Güncel rubrik tahmini (Branch B gerçekleşmiş haliyle)
...
| 2 Çözüm kalitesi (%25) | Fixture bağımsız-oracle'lı (668.75, 3 yoldan doğrulanmış); full-data'da KARAR-0 baseline E1 ihlalini -57.1% azalttığı ÖLÇÜLDÜ; elastik+LNS makinesi kanıtlı çalışıyor (LNS %9.5 gerçek Σslack azalması) | **Full-data'da doğrulanmış objective_value YOK** (Branch B kesinleşti) | **10–15/25** (Branch B'nin dürüst kabulü + üretim merdiveninin "asla ihlalli çıktı yok" garantisi bir miktar telafi ediyor, ama kriterin kendisi doğrudan bir değer istiyor) |
...
| 4 Kod kalitesi (%15) | TDD (348 test), tek komut kurulum+çalıştırma (`KURULUM.md`/`run.sh`), determinizm testi, provenance loglama, `requirements.txt` kesin pin, temiz-klon provası GERÇEKTEN yapıldı (Kapı-9) | 29 script'in bir kısmı M5c-öncesi teşhis kalıntısı (silinmedi, kanıt zinciri için tutuldu) | **12–14/15** (Kapı-0/9'daki hijyen düzeltmeleri + gerçek temiz-klon provası puanı yukarı çekiyor) |
...
**Toplam kaba tahmin**: ~64–82/100 (geniş aralık, esas belirsizlik Kriter
...
test aynı duvara çarparsa alt uca yakın).
...
| 12/12b | Full-ölçekte beklenen çözüm süresi + Gamma'nın büyük ağlarda ölçeklenmesi | `src/config/standard.yaml::gamma` (Gamma değişirse) veya çözüm bütçeleri (`time_limit_sec`/`elastic_time_limit_sec`, süre beklentisi netleşirse) |
...
5. Kaba rubrik tahmini ~64-82/100; en büyük değişken Kriter 2 (full-data
